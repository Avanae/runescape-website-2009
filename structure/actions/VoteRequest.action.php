...
class VoteRequest extends Action
{
    ...
    {
        Registry::get("system")->log("Incoming vote request -> http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], VOTE_LOG);
        ...
        if ($statement == null || !$statement) {
            ...
            Registry::get("system")->log("Voting Request statement error.");
            return;
        }
        if ($site = $statement->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (isset($site['host_name']) && $site['host_name'] != "") {
                ...
                if ($ip_request != $hostip) {
                    ...
                    Registry::get("system")->log("Vote IP mismatch! Ip request = " . $ip_request . " host ip that is validated = " . $hostip . "", VOTE_LOG);
            ...
            $user->write();
            Registry::get("system")->log("The user -> " . ucwords($user->getUsername()) . " voted on " . $site['name'], VOTE_LOG);
            return;
        }
        Registry::get("system")->log("Invalid vote request sent from http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], VOTE_LOG);
    }
