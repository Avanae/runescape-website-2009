...
class EmailValidation extends Action
{
    ...
    {
        $sec = Registry::get("system")->getSecurityManager();
        ...
        $validation;
        if ($emailUpdate) {
            $validation = $system->getValidationManager()->getValidation($cleaned['code']);
            if (!$validation) {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            if ($emailUpdate) {
                $system->getValidationManager()->deleteCode($cleaned['code']);
                $system->sendEmail($user, "Hi " . $user->getFormatUsername() . ",<br>Please visit the following link to update your email address! </br> " . VALIDATE_LINK . Registry::get("system")->getValidationManager()->newValidation($user->getUsername(), 1, $email) . "<br> Kind Regards,<br>" . SITE_NAME . " Account Support", $email);
            ...
            if ($statement->execute(array($email))) {
                if ($statement->rowCount() == 0 || $user->getEmail() == $email) {
                    ...
                    if ($user->write()) {
                        $system->sendEmail($user, "Hi " . $user->getFormatUsername() . ",<br>Please visit the following link to activate your " . SITE_NAME . " account: </br> " . VALIDATE_LINK . Registry::get("system")->getValidationManager()->newValidation($user->getUsername(), 1) . "<br>Kind Regards,<br>" . SITE_NAME . " Account Support");
                        echo "Success! Verification link sent to " . $email;
                    }
                } else {
