...
class Register extends Action
{
    ...
    public function checkRegister($email, $username, $password, $password2, $bot)
    {
        ...
        if (!Registry::get("system")->getSecurityManager()->securityCheck("REGISTRATIONS")) {
        ...
        $uid = $uid->fetch()['UID'];
        Registry::get("system")->log(ucwords($username) . " registered.", 4);
        $_SESSION['uid'] = $uid;
        session_write_close();
        $instanced = User::getByName($username);
        if ($instanced->exists()) {
            Registry::get("system")->sendEmail($instanced, "Hi " . $instanced->getFormatUsername() . ",<br>Please visit the following link to update your email address! </br> " . VALIDATE_LINK . Registry::get("system")->getValidationManager()->newValidation($instanced->getUsername(), 1, $email) . "<br>Kind Regards,<br>" . SITE_NAME . " Account Support", $email);
        }
        return "SUCCESS";
    }
