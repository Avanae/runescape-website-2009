...
class PasswordRecovery extends Action
{
    ...
    public function handle($cleaned)
    {
        ...
        if (!Registry::get("system")->getSecurityManager()->securityCheck("CHANGE_PASSWORD")) {
        ...
        $user->sendEmail("Hi " . $user->getFormatUsername() . "<br><br> We have received a request to reset the password for your " . SITE_NAME . " account.<br><br>To reset your password, click the link below or copy and paste it into your browser: <br><br> " . VALIDATE_LINK . Registry::get("system")->getValidationManager()->newValidation($user->getUsername(), 4) . "<br><br>If you did not submit this password reset request you can safely ignore this email - your account will remain secure.<br>Kind regards,<br>" . SITE_NAME . " Account Support");
    ...
    private function changePassword($code, $password1, $passwordVerify)
    {
        ...
        $validation = Registry::get("system")->getValidationManager()->getValidation($code);
        if (!$validation) {
        ...
        $user->write();
        Registry::get("system")->getValidationManager()->deleteCode($code);
        return "SUCCESS";
