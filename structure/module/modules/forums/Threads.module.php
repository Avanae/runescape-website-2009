...
class Threads extends Module
{
    ...
    {
        if (!Registry::get("system")->getSecurityManager()->securityCheck("FORUM_POST")) {
        ...
        $fm = Registry::get("system")->getForumManager();
    ...
    private function delete($cleaned)
    {
        ...
        if ($this->user->getRights() > 0) {
            Registry::get("system")->log(($isThread ? "Thread" : "Post") . " deleted by " . $this->user->getUsername() . " : " . ($isThread ? ("Thread name=" . $thread->getTitle()) : " post_id=" . $post->getPostId() . " poster_uid=" . $post->getPoster()), FORUM_LOG);
        }
        if ($isThread) {
            $thread->delete();
            return;
    ...
    private function submitPost($cleaned)
    {
        ...
        if (!isset($cleaned['pid']) || $cleaned['pid'] == 0) {
            ...
            if ($user->getRights() > 0) {
                Registry::get("system")->log("Post edited: poster_uid=" . $post->getPoster() . " post_id=" . $post->getPostId() . " edited by: " . $user->getUsername(), FORUM_LOG);
            }
        }
        if (!$statement->execute()) {
            echo "An error occured, please contact a system administrator.";
