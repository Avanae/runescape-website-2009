...
switch ($_GET['action']) {
    ...
    case 'ipn':
        if (!$p->validate_ipn()) {
            $system->log("Paypal: Invalid IPN!", DONATION_LOG);
            return;
        }
        if (!$p->ipn_data["mc_gross"] > -1) {
            $system->log("Paypal: Invalid mc gross data.", DONATION_LOG);
            return;
        }
        $system->log("Paypal: Incoming paypal IPN request.", DONATION_LOG);
        $username = $p->ipn_data["custom"];
        $data = $p->ipn_data["item_number"];
        $amount = $p->ipn_data["mc_gross"];
        $user = User::getByName($username);
        if (!$user) {
            $system->log("Paypal: The username " . $username . " was not found & the IPN could not be processed.", DONATION_LOG);
            return;
        }
        $arr = explode(",", $data);
        $array = array();
        ...
        foreach ($array as $perkId) {
            ...
            if (!$perk) {
                $system->log("Paypal: During the IPN process the perk " . $perkId . " was not found.", DONATION_LOG);
                continue;
            }
        ...
        if (!$user->write()) {
            $system->log("Paypal: Error! Could not write the user " . $username . " profile to the database.", DONATION_LOG);
            return;
        }
        $system->log("Paypal: Donation successfull Username: " . $user->getUsername() . " Perks: " . $data . " Total Price: " . $totalPrice, DONATION_LOG);
        break;
}
